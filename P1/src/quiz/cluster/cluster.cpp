...
// L3-C8:: Fill out this function to return list of indices for each cluster
void clusterHelper(int id, std::vector<std::vector<float>> points, std::vector<bool>& processed, std::vector<int>& cluster, KdTree* tree, float distanceTol)
{
	processed[id] = true;
	cluster.push_back(id);

	std::vector<int> nearPointsId = tree->search(points[id], distanceTol);

	for (int i : nearPointsId)
	{
		if (!processed[i])
		{
